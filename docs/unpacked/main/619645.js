Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.parseGetLinkedGroupClientErrors = function (e) {
  const t = (0, i.parseIQErrorItemNotFoundMixin)(e);
  if (t.success) {
    return (0, a.makeResult)({
      name: "IQErrorItemNotFound",
      value: t.value
    });
  }
  const n = (0, l.parseIQErrorForbiddenMixin)(e);
  if (n.success) {
    return (0, a.makeResult)({
      name: "IQErrorForbidden",
      value: n.value
    });
  }
  const c = (0, r.parseIQErrorBadRequestMixin)(e);
  if (c.success) {
    return (0, a.makeResult)({
      name: "IQErrorBadRequest",
      value: c.value
    });
  }
  const d = (0, u.parseIQErrorNotAllowedMixin)(e);
  if (d.success) {
    return (0, a.makeResult)({
      name: "IQErrorNotAllowed",
      value: d.value
    });
  }
  const f = (0, o.parseIQErrorFallbackClientMixin)(e);
  if (f.success) {
    return (0, a.makeResult)({
      name: "IQErrorFallbackClient",
      value: f.value
    });
  }
  return (0, s.errorMixinDisjunction)(e, ["IQErrorItemNotFound", "IQErrorForbidden", "IQErrorBadRequest", "IQErrorNotAllowed", "IQErrorFallbackClient"], [t, n, c, d, f]);
};
var a = require("../app/135781.js");
var r = require("../app/452390.js");
var o = require("../app/996831.js");
var l = require("../app/341963.js");
var i = require("../app/495837.js");
var u = require("../app/134023.js");
var s = require("../app/686310.js");